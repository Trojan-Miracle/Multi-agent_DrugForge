...
| **chemical_agent** | 计算理化描述符，按 Lipinski / Veber 规则筛选铅化合物 | SMILES 列表 | MW、logP、TPSA、HBD、HBA 等描述符；筛选后铅化合物列表 |
| **admet_properties_agent** | 预测 ADMET 性质和分子对接评分 | SMILES 列表、靶点名称 | 口服生物利用度、溶解度、hERG、肝毒性等；对接评分 |
| **molecule_optimization_agent** | 针对指定性质优化铅化合物结构 | SMILES + 目标性质 + 方向（increase/decrease） | 优化后 SMILES |
| **trial_generator_agent** | 生成结构化临床试验方案 | 铅化合物 SMILES / 药物名称 | 包含分期、入组、终点、臂等完整试验文本 |
