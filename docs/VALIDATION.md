...
已执行的真实 API、RDKit、对接及公开数据基线实验详见[科学验证记录](SCIENCE_VALIDATION.md)，与上述离线流程验证分开报告。

1. 部分领域连接器已完成真实调用，完整模型链路尚未完成联调；权重、API 及对接工具需分别配置。新版依赖与兼容性例外见[版本记录](DEPENDENCIES.md)。
